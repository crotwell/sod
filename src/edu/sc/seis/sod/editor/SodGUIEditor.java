...
public class SodGUIEditor extends SimpleGUIEditor {
    ...
    public SodGUIEditor(String[] args) throws IOException,
            ParserConfigurationException, TransformerException, DOMException,
    ...
    JComponent getCompForElement(Element element) {
        try {
            ...
            System.err.println("Can't get component for "
                    + element.getTagName() + ", using default. " + e);
            logger.warn("Can't get component for " + element.getTagName()
                    + ", using default. ", e);
            return super.getCompForElement(element);
        }
    }
    ...
    protected void initEditors() {
        ...
        editors.put("printlineEventProcess", new PrintlineEventEditor());
        ...
        editors.put("originDepthRange", new UnitRangeEditor(DISTANCE_UNITS,
                                                            true));
        editors.put("linearDistanceMagnitudeRange",
        ...
        editors.put("latitudeRange", new UnitRangeEditor(ANGLE_UNITS,
                                                         ...
                                                         5,
                                                         false));
        editors.put("longitudeRange", new UnitRangeEditor(ANGLE_UNITS,
                                                          -180,
                                                          180,
                                                          5,
                                                          false));
        editors.put("azimuthRange", new UnitRangeEditor(ANGLE_UNITS,
                                                        0,
                                                        360,
                                                        5,
                                                        true));
        editors.put("backAzimuthRange", new UnitRangeEditor(ANGLE_UNITS,
                                                            0,
                                                            360,
                                                            5,
                                                            true));
        ...
        // go in this place
        /*
         * COMMENTED OUT 2005/05/20 because switchers don't work for (int i = 0;
         * i < switchTypes.length; i++) { Switcher originSwitcher = new
         * Switcher(switchTypes[i], this); List subTypes =
         * originSwitcher.getSubTypes(); Iterator it = subTypes.iterator();
         * while(it.hasNext()) { String tagName = (String)it.next(); if
         * (editors.containsKey(tagName)) { // save original with _tagChooser
         * appended if (editors.containsKey(tagName+TagChooser.PLUGIN_SUFFIX)) { //
         * already visited this editor plugin } else { if ( !
         * (editors.get(tagName) instanceof Switcher)) { // only add switch if
         * current plugin isn't a switcher
         * editors.put(tagName+TagChooser.PLUGIN_SUFFIX, editors.get(tagName));
         * editors.put(tagName, originSwitcher); } } } else { // no custom
         * editor editors.put(tagName, originSwitcher); } } }
         */
    }
