...
public abstract class ServerEditor {
    ...
    protected abstract ServerNameDNS getServerNameDNS(String name,
                                                      String dns,
                                                      FissuresNamingService fisName);

    protected abstract ServerNameDNS[] getAllServers(FissuresNamingService fisName);

    protected Box getServerBox(final Element element)
            throws TransformerException {
        Box serverBox = Box.createHorizontalBox();
        serverBox.setBorder(new TitledBorder("Server"));
        ServerNameDNSWrapper current = new ServerNameDNSWrapper(getServerNameDNS(XPathAPI.selectSingleNode(element,
                                                                                                           "dns/text()")
                                                                                         .getNodeValue(),
                                                                                 XPathAPI.selectSingleNode(element,
                                                                                                           "name/text()")
                                                                                         .getNodeValue(),
                                                                                 fisName));
        ...
        combo.addItemListener(new ItemListener() {
            ...
            public void itemStateChanged(ItemEvent e) {
                if(e.getStateChange() == ItemEvent.SELECTED) {
                    ...
                    try {
                        XPathAPI.selectSingleNode(element, "dns/text()")
                                .setNodeValue(selected.getServerDNS());
                        XPathAPI.selectSingleNode(element, "name/text()")
                                .setNodeValue(selected.getServerName());
                    } catch(TransformerException ex) {
                        // oh well?
                        GlobalExceptionHandler.handle("Can't update server in XML",
                                                      ex);
                    }
                }
            }
    ...
    class ServerNameDNSWrapper implements ServerNameDNS {
        ...
        public String toString() {
            return SimpleGUIEditor.getDisplayName(server.getServerDNS() + "/"
