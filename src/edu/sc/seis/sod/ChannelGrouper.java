...
public class ChannelGrouper {
	...
	private ChannelGroup[] applyRules(Channel[] channels, Element[] rules,List failures){
	...
	private List getGroupableChannels(LinkedList chn,Element[] rules,List failedList) {
	...
	private Element[] loadRules(String configFileLoc) {
		...
		try {
			...
			GlobalExceptionHandler.handle("Invalid config File " + configFileLoc + " for Channel Grouper",e);
			return new Element[0];
		} catch (SAXException e) {
			GlobalExceptionHandler.handle("Invalid config File " + configFileLoc + " for Channel Grouper",e);
			return new Element[0];
		} catch (IOException e) {
			GlobalExceptionHandler.handle("Config File for Channel Grouper " + configFileLoc + " not found ",e);
			return new Element[0];
		}
		return rules;
	}
