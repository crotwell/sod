...
public class MidPoint implements EventChannelSubsetter {
    ...
    public boolean accept(EventAccessOperations eventAccess, Channel chan, CookieJar cookieJar)
        ...
        if(area instanceof edu.iris.Fissures.BoxArea) {
            ...
            pDist.min_distance = QuantityImpl.createQuantityImpl(pDist.min_distance).convertTo(UnitImpl.DEGREE);
            pDist.max_distance = QuantityImpl.createQuantityImpl(pDist.max_distance).convertTo(UnitImpl.DEGREE);
            double midDist = SphericalCoords.distance(latitude, longitude, pDist.latitude, pDist.longitude);
            if (midDist >= pDist.min_distance.value && midDist <= pDist.max_distance.value) {
                return true;
            } else {
                return false;
            }
