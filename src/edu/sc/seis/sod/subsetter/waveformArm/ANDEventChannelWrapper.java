...
public class ANDEventChannelWrapper implements EventChannelGroupSubsetter {
    ...
    public ANDEventChannelWrapper(Element config) throws ConfigurationException{
        ...
        for(int counter = 0; counter < childNodes.getLength(); counter++) {
            ...
            if(node instanceof Element) {
                subsetter =
                    (EventChannelSubsetter) SodUtil.load((Element)node, "waveformArm");
                break;
            }
        }
    }
    ...
    public boolean accept(EventAccessOperations event, ChannelGroup channelGroup, CookieJar cookieJar) throws Exception {
        for (int i = 0; i < channelGroup.getChannels().length; i++) {
            if ( ! subsetter.accept(event, channelGroup.getChannels()[i], cookieJar)) {
                return false;
            }
        }
        return true;
