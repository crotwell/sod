        ...
        for(int i = 0; i < nets.length; i++) {
            ...
            for(int j = 0; j < stas.length; j++) {
                ...
                for(int k = 0; k < sites.length; k++) {
                    ChannelDbObject[] chans = arm.getSuccessfulChannels(nets[i], sites[k]);
                    for(int l = 0; l < chans.length; l++) {
                        MicroSecondTimeRange chanRange = new MicroSecondTimeRange(chans[l].getChannel().effective_time);
                        if(range == null) {
                            range = chanRange;
                        } else {
                            if(chanRange.getBeginTime().before(range.getBeginTime())) {
