...
public abstract class AbstractVelocityStatus implements WaveformArmMonitor,
    ...
    public AbstractVelocityStatus(String fileDir, String templateName)
        ...
        try {
            Element menuEl = TemplateFileLoader.getTemplate(
                    getClass().getClassLoader(), MenuTemplate.TEMPLATE_LOC);
            MenuTemplate menu = new MenuTemplate(menuEl, createExamplePath(),
                    fileDir);
            renderedMenu = menu.getResult();
        } catch (Exception e) {
            GlobalExceptionHandler.handle(
                    "Trouble creating menuTemplate for Velocity based status",
                    e);
        }
    }
    ...
    protected String loadTemplate(String templateName) throws IOException {
        URL templateURL = TemplateFileLoader.getUrl(
                this.getClass().getClassLoader(), templateName);
    ...
    public void scheduleOutput(String filename, Context context, String template) {
        OutputScheduler.getDefault().schedule(
                new VelocityRenderer(filename, context, template));
    }
