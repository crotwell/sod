...
LIB=C:\cygwin\home\groves\repository


ITEXT=${LIB}/itext/jars/itext-0.99.jar
...
FISSURESUTIL=${LIB}/fissuresUtil/jars/fissuresUtil-1.0.9beta.jar
FISSURESIMPL=${LIB}/fissuresImpl/jars/fissuresImpl-1.1.7beta.jar
FISSURESIDL=${LIB}/fissuresIDL/jars/fissuresIDL-1.0.jar
SEEDCODEC=${LIB}/SeedCodec/jars/SeedCodec-1.0beta2.jar
TAUP=${LIB}/TauP/jars/TauP-1.1.5beta.jar
JUNIT_ADDONS=${LIB}/junit-addons/jars/junit-addons-1.4.jar
JUNIT=${LIB}/junit/jars/junit-3.8.1.jar
HSQLDB=${LIB}/hsqldb/jars/hsqldb-1.7.2.jar
...
ETOPO10=${LIB}/mapData/jars/etopo10.jar
SOD=${LIB}/sod/jars/sod-2.0beta.jar

CLASSPATH=${ITEXT}:${ISTI_UTIL}:${ISTI_UTIL_TOPLEVEL}:${JDOM}:${JING}:${OPENMAP}:${JCALENDAR}:${JACORB}:${IDL}:${ANTLR}:${AVALON_FRAMEWORK}:${CONCURRENT}:${LOGKIT}:${FISSURESUTIL}:${FISSURESIMPL}:${FISSURESIDL}:${SEEDCODEC}:${TAUP}:${JUNIT_ADDONS}:${JUNIT}:${HSQLDB}:${XERCES}:${XML_APIS}:${XALAN}:${LOG4J}:${RNGCONV}:${MSV}:${ISORELAX}:${RELAXNGDATATYPE}:${XSDLIB}:${VELOCITY}:${COMMONS_COLLECTIONS}:${VELOCITY_TOOLS_GENERIC}:${JAX_QNAME}:${JSR173_API}:${JSR173_RI}:${NAMESPACE}:${ETOPO10}:${SOD}
